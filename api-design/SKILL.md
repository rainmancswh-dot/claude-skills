...
If CONTEXT.md exists, read it and use its terminology for all entity naming, field naming, and endpoint descriptions. Do not invent new synonyms for terms already defined.
...
### 1.5 Scan Existing API Patterns
...
### 1.6 Check Existing api-contract Files

```bash
if [ -f "$PROJECT_ROOT/docs/api-contract.md" ]; then
  echo "EXISTING_CONTRACT: $PROJECT_ROOT/docs/api-contract.md"
fi
if [ -f "$PROJECT_ROOT/docs/api-contract.yaml" ]; then
  echo "EXISTING_OPENAPI: $PROJECT_ROOT/docs/api-contract.yaml"
fi
```
...
Write to `$PROJECT_ROOT/docs/api-contract.md`:
...
Write OpenAPI 3.0 spec to `$PROJECT_ROOT/docs/api-contract.yaml`:
...
grep -n '"data": \[' "$PROJECT_ROOT/docs/api-contract.md"
# Any match = VIOLATION. Fix before proceeding.

# Check for bare arrays in yaml (violations = missing pagination)
grep -n 'type: array' "$PROJECT_ROOT/docs/api-contract.yaml" | grep -v 'items:' | head -5
# If data is type: array directly under a response (not under list:), that's a VIOLATION.
```

**How to check rule 2 (DELETE body):**
```bash
grep -B2 "requestBody" "$PROJECT_ROOT/docs/api-contract.yaml" | grep -i delete
# Any match = VIOLATION.
```

**How to check rule 3 (schema completeness):**
```bash
grep -c '"description":' "$PROJECT_ROOT/docs/api-contract.yaml"
grep -c 'schema:' "$PROJECT_ROOT/docs/api-contract.yaml"
# Every response must have both. Spot-check visually.
```

**How to check rule 4 (security):**
```bash
head -15 "$PROJECT_ROOT/docs/api-contract.yaml" | grep "security:"
# Must be present.
```
...
if command -v npx >/dev/null 2>&1; then
  npx --yes @redocly/cli lint "$PROJECT_ROOT/docs/api-contract.yaml" 2>&1 | head -20 || true
fi
```
...
```bash
cd "$PROJECT_ROOT"
git add docs/api-contract.md docs/api-contract.yaml
...
| Consumer | File | How |
|----------|------|-----|
| `/flow-extract` | docs/api-contract.md | Read interface semantics to generate interaction flows |
| `/write-plan` or subagent-develop | docs/api-contract.yaml | Generate TypeScript types and API client code |
| Playwright test generation | docs/api-contract.yaml | Generate mock data for test scenarios |
